...
public class Formula {
    ...
    // Splitet ein Feld in mehrere Felder an den Stellen wo eine Special nodes.Node ist damit man danach
    // mehrere Felder hat, die entweder eine Special nodes.Node beinhalten oder aus mehreren Normalen Nodes bestehen.
    // Fügt außerdem ein mal zwischen diese Felder ein, falls es eine Value nodes.Node und eine Special nodes.Node nebeneinander gibt.
    ...
    // Verbindet benachbarte Value Nodes in eine einzige Value nodes.Node mit dem Wert von allen ursprünglich benachbarten Value Nodes.
    private void combineValueNodes(Field field) {
        ...
        for (int i = 0; i < field.getLength(); i++) {
            if (currentValueNode == null && field.getNode(i).getType() == Constants.Type.Value) {
                System.out.println("Set new Value nodes.Node");
                ...
                System.out.println("Reset Value nodes.Node");
    ...
    private Field addPowerNodes(Field field) {
        ...
        System.out.println("Power nodes.Node 1: ");
        System.out.println(field.getLength());
        for (int i = 0; i < field.getLength(); i++) {
        ...
        System.out.println("Power nodes.Node 2: ");
        System.out.println(field.getLength());
        for (int i = 0; i < nodes.size(); i++) {
            System.out.println("currentNode: " + nodes.get(i).getType());
        }

        System.out.println("Power nodes.Node 3: ");
        System.out.println(field.getLength());
    ...
    public ArrayList<Node> toPolishNotation(Field field) {
        ...
        for (int i = 0; i < inputNodes.size(); i++) {
            if (inputNodes.get(i).getType() == Constants.Type.Value) { // Wenn die nodes.Node eine Value nodes.Node ist
                System.out.println("Value nodes.Node: " + inputNodes.get(i).getValue());
                outputQueue.add(inputNodes.get(i));
            }
            else if (inputNodes.get(i).getNodeType() == Constants.NodeType.Normal) { // Wenn die nodes.Node eine Basic nodes.Node ist aber keine Value nodes.Node
                System.out.println("Normale nodes.Node: " + inputNodes.get(i).getType());
                if (inputNodes.get(i).getType() == Constants.Type.OpeningBracket || inputNodes.get(i).getType() == Constants.Type.ClosingBracket) {
                    System.out.println("Bracket nodes.Node");
            ...
            else if (inputNodes.get(i).getNodeType() == Constants.NodeType.Special) { // Wenn die nodes.Node eine Special nodes.Node ist
                System.out.println("Special nodes.Node");
            ...
            } else if (inputNodes.get(i).getNodeType() == Constants.NodeType.ExponentSpecial) { // Wenn die nodes.Node eine Special nodes.Node ist
                System.out.println("Single Special nodes.Node");
                ExponentSpecialNode exponentSpecialNode = (ExponentSpecialNode)inputNodes.get(i);

                System.out.println("Child calculation.Field");
                for (int j = 0; j < exponentSpecialNode.getChildField().getLength(); j++) {
                    System.out.println("CF" + exponentSpecialNode.getChildField().getNode(j).getType());
                }

                System.out.println("Value calculation.Field");
                for (int j = 0; j < exponentSpecialNode.getValueField().getLength(); j++) {
                    System.out.println("VF" + exponentSpecialNode.getValueField().getNode(j).getType());
                }
        ...
        for (int i = 0; i < outputQueue.size(); i++) {
            System.out.println("output nodes.Node: " + outputQueue.get(i).getType());
        }
        return outputQueue;
    }
