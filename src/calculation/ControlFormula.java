...
public class ControlFormula {
    ...
    public void addOperator(Constants.Type type) {
        ...
        } else if (type == Constants.Type.Logaritm)  {
            LogaritmNode logaritmNode = new LogaritmNode(currentField);
    ...
    public void moveCursorRight() {
        ...
        if (cursorPosition == currentField.getLength() - 1 && currentField.hasParent()) {
            System.out.println("At calculation.Field End");
            if (currentField.getParent().getNodeType() == Constants.NodeType.Special) {
                SpecialNode parentNode = (SpecialNode) currentField.getParent(); // Single Special nodes.Node wird noch nicht beachtet
                if (currentField.equals(parentNode.getFirstChild())) { // Move Cursor to Second calculation.Field
                    System.out.println("Move to Second calculation.Field");
                    currentField = parentNode.getSecondChild();
                    cursorPosition = 0;
                } else { // Move Cursor out of calculation.Field
                    System.out.println("Move out of calculation.Field");
            ...
            if (currentField.getNode(cursorPosition - 1).getNodeType() == Constants.NodeType.Special) { // Move Cursor in calculation.Field
                System.out.println("Move Cursor in calculation.Field");
    ...
    public void moveCursorLeft() {
        ...
        if (cursorPosition == 0 && currentField.hasParent()) {
            System.out.println("At calculation.Field Beginning");
            if (currentField.getParent().getNodeType() == Constants.NodeType.Special) {
                SpecialNode parentNode = (SpecialNode) currentField.getParent();
                if (currentField.equals(parentNode.getSecondChild())) { // Move Cursor to First calculation.Field
                    System.out.println("Move to First calculation.Field");
                ...
                ExponentSpecialNode parentNode = (ExponentSpecialNode) currentField.getParent(); // Single Special nodes.Node wird noch nicht beachtet
                System.out.println("Move out of calculation.Field");
                currentField = parentNode.getParentField();
                cursorPosition = parentNode.getParentField().getIndexOf(parentNode);
            } else if (currentField.getParent().getNodeType() == Constants.NodeType.SingleSpecial) {
                SingleSpecialNode parentNode = (SingleSpecialNode) currentField.getParent(); // Single Special nodes.Node wird noch nicht beachtet
                System.out.println("Move out of calculation.Field");
                currentField = parentNode.getParentField();
                cursorPosition = parentNode.getParentField().getIndexOf(parentNode);
            }
            ...
            if (currentField.getNode(cursorPosition).getNodeType() == Constants.NodeType.Special) { // Move Cursor in calculation.Field
                System.out.println("Move Cursor in calculation.Field");
    ...
    public void setCursorPosition(Node clickedNode) {
        ...
        cursorPosition = clickedNode.getParentField().getIndexOf(clickedNode) + 1; // +1 damit man vor der nodes.Node ist auf die man Klickt
        renderAll();
    }
    ...
    public void calculate() {
        ...
        System.out.println("--------calculation.Equation Start--------");
